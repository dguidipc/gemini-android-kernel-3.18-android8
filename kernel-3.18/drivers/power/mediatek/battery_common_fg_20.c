...
void do_chrdet_int_task(void)
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			//#ifndef CONFIG_MTK_RT9466_SUPPORT
			if(chargin_hw_init_done_rt != KAL_TRUE)
			...
			battery_log(BAT_LOG_CRTI,
				    "[do_chrdet_int_task] call mt_battery_charger_detect_check.\n");

			if (BMT_status.bat_vol > 0){
				mt_battery_update_status();
				battery_log(BAT_LOG_CRTI,
					    "[do_chrdet_int_task] call mt_battery_update_status() in EVB\n");
			}
		}
#endif
...
int bat_routine_thread(void *x)
{
	...
	while (1) {
		...
		if ((((chargin_hw_init_done == KAL_TRUE) && (battery_suspended == KAL_FALSE))
			...
			|| (((chargin_hw_init_done_rt == KAL_TRUE) && (battery_suspended == KAL_FALSE)) 
			|| ((chargin_hw_init_done_rt == KAL_TRUE) && (chr_wake_up_bat == KAL_TRUE))))
			BAT_thread();
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	do {
		...
		if (chargin_hw_init_done || chargin_hw_init_done_rt)
