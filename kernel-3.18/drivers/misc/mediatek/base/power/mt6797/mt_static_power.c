...
static struct sptab_s sptab[MT_SPOWER_MAX]; /* CPU, GPU, CPUL, CPULL, VCORE, MODEM, VMD1  */
...
int mt_spower_make_table(struct sptab_s *spt, struct spower_raw_s *spower_raw, int wat, int voltage, int degree)
{
	int i;
	int c1, c2, c = -1;
...
void mt_spower_ut(void)
{
	...
	for (i = 0; i < MT_SPOWER_MAX; i++) {
		...
		case  MT_SPOWER_CPUBIG:
			SPOWER_INFO("[SPOWER] - This is MT_SPOWER_CPUBIG\n");
			break;
		case  MT_SPOWER_GPU:
...
int mt_spower_init(void)
{
	...
	if (tab_validate(&sptab[MT_SPOWER_CPUBIG]))
		return 0;
