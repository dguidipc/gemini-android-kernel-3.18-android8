...
{
	ssize_t _tLength = 0;
	struct acc_hw *_ptAccelHw = &obj_i2c_data->hw;
...
static int icm20645_factory_do_self_test(void)
{
	return 0;
}
...
static int icm20645_factory_set_cali(int32_t data[3])
{
	int err = 0;
	int cali[3] = { 0 };
	struct icm20645_i2c_data *obj = obj_i2c_data;
...
static int icm20645_factory_enable_calibration(void)
{
	return 0;
}
static int icm20645_factory_clear_cali(void)
{
	int err = 0;

	err = ICM20645_ResetCalibration(icm20645_i2c_client);
	if (err) {
		GSE_ERR("icm20645_factory_clear_cali failed!\n");
		return -1;
	}
	return 0;
}

static int icm20645_factory_get_raw_data(int32_t data[3])
{
	char strbuf[ICM20645_BUFSIZE] = { 0 };

	ICM20645_ReadRawData(icm20645_i2c_client, strbuf);
	GSE_LOG("support icm20645_factory_get_raw_data!\n");
	return 0;
}

static int icm20645_factory_get_data(int32_t data[3], int *status)
{
	return icm20645_get_data(&data[0], &data[1], &data[2], status);

}

static int icm20645_factory_enable_sensor(bool enabledisable, int64_t sample_periods_ms)
{
	int err;
	...
	err = icm20645_batch(0, sample_periods_ms * 1000000, 0);
	if (err) {
...
static struct accel_factory_fops icm20645_factory_fops = {
	.enable_sensor = icm20645_factory_enable_sensor,
	.get_data = icm20645_factory_get_data,
	.get_raw_data = icm20645_factory_get_raw_data,
	.enable_calibration = icm20645_factory_enable_calibration,
	.clear_cali = icm20645_factory_clear_cali,
	.set_cali = icm20645_factory_set_cali,
	.get_cali = icm20645_factory_get_cali,
	.do_self_test = icm20645_factory_do_self_test,
};

static struct accel_factory_public icm20645_factory_device = {
	.gain = 1,
	.sensitivity = 1,
	.fops = &icm20645_factory_fops,
};
...
static int icm20645_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* factory */
	err = accel_factory_device_register(&icm20645_factory_device);
	if (err) {
	...
	ctl.batch = icm20645_batch;
	ctl.flush = icm20645_flush;
...
exit_misc_device_register_failed:
	accel_factory_device_deregister(&icm20645_factory_device);
exit:
	GSE_ERR("%s: err = %d\n", __func__, err);
