...
static int BMI160_ACC_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if (enable == true)
		...
		databuf[0] = CMD_PMU_ACC_NORMAL/*CMD_PMU_ACC_SUSPEND*/;
 ...
 *
*/
BMI160_RETURN_FUNCTION_TYPE bmi160_set_intr_edge_ctrl_i2c(
 ...
 *
*/
BMI160_RETURN_FUNCTION_TYPE bmi160_set_intr_level_i2c(
...
*
*/
BMI160_RETURN_FUNCTION_TYPE bmi160_set_output_enable_i2c(
...
static int bmi160acc_setup_eint(struct i2c_client *client)
{
	...
	if (node) {
		...
		if (!obj->IRQ) {
			...
			return -EINVAL;
		}//IRQ_TYPE_LEVEL_LOW IRQF_TRIGGER_RISING
		if (request_irq
...
static int bmi160_acc_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GSE_ERR("endy,%s obj->hw->direction = %d.\n",__func__,obj->hw->direction);
	...
	bmi160_set_intr_edge_ctrl_i2c(BMI_INT0, BMI_INT_LEVEL);
	udelay(500);
	bmi160_set_intr_fifo_wm(BMI_INT0, ENABLE);
	udelay(500);
	bmi160_set_intr_level_i2c(BMI_INT0, ENABLE);
	udelay(500);
	bmi160_set_output_enable_i2c(BMI160_INTR1_OUTPUT_ENABLE, ENABLE);
	udelay(500);
