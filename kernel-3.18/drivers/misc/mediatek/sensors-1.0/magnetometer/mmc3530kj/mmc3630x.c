...
/* Log define */
#define MEMSIC_INFO(fmt, arg...)      	pr_warn("<<-MMC3630X INFO->> "fmt"\n", ##arg)
#define MEMSIC_ERR(fmt, arg...)          	pr_err("<<-MMC3630X ERROR->> [line=%d]"fmt"\n",__LINE__,##arg)
#define MEMSIC_DEBUG(fmt, arg...)		do {\
						if (MEMSIC_DEBUG_ON)\
							pr_warn("<<-MMC3630X DEBUG->> [%d]"fmt"\n", __LINE__, ##arg);\
					} while (0)
...
static int mmc3630x_suspend(struct i2c_client *client, pm_message_t msg);
static int mmc3630x_resume(struct i2c_client *client);
...
static const struct of_device_id mag_of_match[] = {
	{.compatible = "mediatek,msensor_mmc3530kj"},
	{},
};
#endif
...
static int ECS_ReadXYZData(int *vec, int size)
{
	...
	if (!(read_idx % MMC3630X_RESET_INTV))
	{
		...
		data[0] = 0x07;//MMC3630X_REG_CTRL;
		data[1] = 0x80;//MMC3630X_CTRL_REFILL;
...
#if MEMSIC_SINGLE_POWER
    tmp[0] =0x07;// MMC3630X_REG_CTRL;
	tmp[1] = 0x80;//MMC3630X_CTRL_REFILL;
	I2C_TxData(tmp, 2);
	msleep(MMC3630X_DELAY_SET);
#endif
	tmp[0] = 0x0F;
...
{
#if 0
	const char *name = "mediatek,msensor_mmc3530kj";

	hw = get_mag_dts_func(name, hw);

	MEMSIC_INFO("mmc3630x_init addr0 = 0x%x,addr1 = 0x%x,i2c_num = %d \n",hw->i2c_addr[0],hw->i2c_addr[1],hw->i2c_num);
