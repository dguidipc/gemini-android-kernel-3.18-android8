...
static void mtk_enable_otg_mode(void)
{
    ...
    printk("%s().\n",__func__);    
    aeon_gpio_set("usb0_drvvbus_high");

#if defined(CONFIG_MTK_BQ25898_DUAL_SUPPORT)
	bq25898_otg_en(0x01);
	bq25898_set_boost_ilim(0x01);
#else
#if defined(CONFIG_MTK_BQ25896_SUPPORT)|| defined(CONFIG_MTK_RT9466_SUPPORT)
		if(chargin_hw_init_done_rt == 1)
		{
			set_chr_enable_otg(0x01);
...
{
    printk("%s().\n",__func__);
    aeon_gpio_set("usb0_drvvbus_low");
