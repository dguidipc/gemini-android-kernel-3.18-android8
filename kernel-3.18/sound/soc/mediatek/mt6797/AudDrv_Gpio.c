...
/*	[GPIO_I2S_MODE0] = {"audi2s1_mode0", false, NULL},
	...
	[GPIO_HPDEPOP_E2_HIGH] = {"hpdepop-pullhigh_e2", false, NULL},
	[GPIO_HPDEPOP_E2_LOW] = {"hpdepop-pulllow_e2", false, NULL},
		...
		if (aud_gpios[GPIO_HPDEPOP_HIGH].gpio_prepare) {
			for (i = 0; i < extamp_mode; i++) {
				retval = pinctrl_select_state(pinctrlaud,
						aud_gpios[GPIO_HPDEPOP_LOW].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_EXTAMP_LOW] pins\n");
				udelay(2);
				retval = pinctrl_select_state(pinctrlaud,
						aud_gpios[GPIO_HPDEPOP_HIGH].gpioctrl);
				if (retval)
...
int AudDrv_GPIO_EXTAMP2_Select(int bEnable, int mode)
{
	...
	if (bEnable == 1) {
		...
		if (aud_gpios[GPIO_HPDEPOP_E2_HIGH].gpio_prepare) {
			for (i = 0; i < extamp_mode; i++) {
				retval = pinctrl_select_state(pinctrlaud,
						aud_gpios[GPIO_HPDEPOP_E2_LOW].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_HPDEPOP_E2_LOW] pins\n");
				udelay(2);
				retval = pinctrl_select_state(pinctrlaud,
						aud_gpios[GPIO_HPDEPOP_E2_HIGH].gpioctrl);
				if (retval)
					pr_err("could not set aud_gpios[GPIO_HPDEPOP_E2_HIGH] pins\n");
				udelay(2);
			}
		}
	} else {
		if (aud_gpios[GPIO_HPDEPOP_E2_LOW].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_HPDEPOP_E2_LOW].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_HPDEPOP_E2_LOW] pins\n");
		}
...
int AudDrv_GPIO_HPDEPOP_Select(int bEnable)
{
	...
	if (bEnable == 0){
	{
		...
		AudDrv_GPIO_Select(GPIO_HPDEPOP_E2_LOW);
	}
	else{
		AudDrv_GPIO_Select(GPIO_HPDEPOP_HIGH);
		AudDrv_GPIO_Select(GPIO_HPDEPOP_E2_HIGH);
        }
#endif
	mutex_unlock(&gpio_request_mutex);
